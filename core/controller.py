...
from .state import DebugState, DebugStatus, StopReason, Breakpoint, SourceLocation
...
class DebugController:
    ...
    def start(self, config: DebugConfig) -> bool:
        ...
        try:
            ...
            self.state.status = DebugStatus.CONFIGURED
    ...
    def pause(self) -> bool:
        ...
        Returns:
            bool: 是否成功
        """
        if self.state.status != DebugStatus.RUNNING:
            logger.warning("Debug session is not running")
            return False
    ...
    def step_over(self) -> bool:
        ...
        Returns:
            bool: 是否成功
        """
        if not self._can_control(require_thread=True):
            return False
    ...
    def _can_control(self, require_thread: bool = False) -> bool:
        """检查是否可以执行控制命令"""
        if not self._initialized:
            logger.warning("Debug session is not started")
            return False
        if self.state.status != DebugStatus.STOPPED:
            logger.warning("Debug session is not stopped")
            return False
        if require_thread and self.state.current_thread_id is None:
            logger.warning("No active stopped thread")
            return False
        return True
    ...
    def set_breakpoint(self, file_path: str, line: int,
        ...
        if not self._initialized:
            pending = Breakpoint(
                verified=False,
                source=SourceLocation(path=file_path, line=line),
                line=line,
                condition=condition,
                hit_condition=hit_condition,
                log_message=log_message,
            )
            existing = [b for b in self.state.get_file_breakpoints(file_path) if b.line != line]
            existing.append(pending)
            self.state.breakpoints[file_path] = existing
