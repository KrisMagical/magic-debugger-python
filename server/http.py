...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        status, result = self.api_server.handle_request("GET", path, query=query)
    ...
    def do_POST(self):
        ...
        try:
            ...
            self._send_json(400, error_payload("BAD_REQUEST", "Invalid JSON"))
            return
        
        status, result = self.api_server.handle_request("POST", path, payload=data)
        self._send_json(status, result)
...
class HTTPAPIServer:
    ...
    def handle_request(
        ...
        """Handle one API request and return (status_code, payload)."""
        handler = self.router.get_handler(method, path)
        if not handler:
            return 404, error_payload("NOT_FOUND", "Not found")

        try:
            result = handler(query or {}) if method == "GET" else handler(payload or {})
            return normalize_controller_result(result)
        except Exception as exc:
            logger.error(f"Error handling {method} {path}: {exc}")
    ...
    def start(self, host: str = "127.0.0.1", port: int = 8765) -> bool:
        ...
        try:
            ...
            HTTPRequestHandler.api_server = self
