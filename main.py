...
from server.rpc import DEFAULT_RPC_HOST, DEFAULT_RPC_PORT, RPCServer
...
class MagicDebug:
    ...
    def initialize(self) -> bool:
        ...
        if not self.rpc_server.start(
            transport=self.config.get("rpc_transport", "auto"),
            socket_path=self.config.get("rpc_socket_path") or self.config.get("socket_path"),
            host=self.config.get("rpc_host", DEFAULT_RPC_HOST),
            port=self.config.get("rpc_port", DEFAULT_RPC_PORT),
        ):
        ...
        logger.info(f"RPC endpoint: {self.rpc_server.endpoint.url()}")
...
Examples:
  ...
  %(prog)s --rpc-transport tcp      # Use TCP RPC
  %(prog)s --rpc-socket /tmp/md.sock # Custom Unix socket path
  %(prog)s --port 9000              # Custom HTTP port
    ...
    parser.add_argument(
        "--rpc",
        action="store_true",
        help="Start RPC server mode (default behavior; kept for plugin compatibility)"
    )
    parser.add_argument(
        "--args", "-a",
        metavar="ARGS",
    ...
    parser.add_argument(
        "--socket", "-s",
        metavar="PATH",
        help="Deprecated alias for --rpc-socket"
    )
    parser.add_argument(
        "--rpc-transport",
        choices=["auto", "unix", "tcp"],
        default="auto",
        help="RPC transport: auto, unix, or tcp (default: auto)"
    )
    parser.add_argument(
        "--rpc-socket",
        metavar="PATH",
        help="Unix socket path for RPC"
    )
    parser.add_argument(
        "--rpc-host",
        default=DEFAULT_RPC_HOST,
        metavar="HOST",
        help=f"TCP RPC host (default: {DEFAULT_RPC_HOST})"
    )
    parser.add_argument(
        "--rpc-port",
        type=int,
        ...
        metavar="PORT",
        help=f"TCP RPC port (default: {DEFAULT_RPC_PORT})"
    )
    parser.add_argument(
        "--port", "-p",
        type=int,
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Emit JSON for --check or doctor"
    )
    parser.add_argument(
        "--version",
        action="version",
    ...
    # 检查安装
    if args.check or args.command == "doctor":
        report = run_doctor(
            rpc_transport=args.rpc_transport,
            rpc_host=args.rpc_host,
            rpc_port=args.rpc_port,
            rpc_socket_path=args.rpc_socket or args.socket,
            http_port=args.port,
        )
        print(report_to_json(report) if args.json else format_doctor_report(report))
        return result_exit_code(report["result"])
