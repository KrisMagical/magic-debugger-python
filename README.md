...
> **注意**：需要 GDB 版本 >= 11 以支持 DAP 协议。可使用 `gdb --version` 检查版本。
...
optional arguments:
  ...
  --check               Check GDB installation and exit
...
- 确保 GDB 版本 >= 11（`gdb --version`）。
- 运行 `magic-debug --check` 查看 GDB 状态。
- 若路径不正确，可在配置中手动指定 `gdb_path`。
